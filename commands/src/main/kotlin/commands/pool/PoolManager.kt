...
class PoolManager private constructor() {
    ...
    fun registerPool(message: IMessage, pool: Pool) {
        ...
        Timer("Pool timer" + pool.id(), true)
                .schedule(PoolFinalizerTask(pool), TimeUnit.MILLISECONDS.convert(pool.time(), TimeUnit.SECONDS))
    }


    private class PoolFinalizerTask(private val pool: Pool) : TimerTask() {
        ...
        override fun run() {
            ...
            val currentMessage = channel.getMessageByID(this.pool.message().id)
            ...
            if (pool.type() == Pool.Type.MULTI) {
                for ((key) in options) {
                    val reaction = currentMessage.getReactionByName(EmojiManager.getForAlias(key).unicode)
                    if (reaction != null) {
                        try {
            ...
            try {
                ...
                MessageUtils.handleDiscord4JException(logger, e, PoolCommand.instance, currentMessage)
